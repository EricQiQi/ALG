...
大白话：**前缀和就是"从根一路加到当前节点"的总和，走到哪就加到哪。**

```
路径：10 → 5 → 3

走到 10：prefixSum = 10          （从根加到 10）
走到 5： prefixSum = 10+5 = 15   （从根加到 5）
走到 3： prefixSum = 10+5+3 = 18 （从根加到 3）
```

两个前缀和一减，掐掉前面重复的部分，剩下的就是中间那段：
...
```
  ...
  prefixSum(3) - prefixSum(10) = 18 - 10 = 8
  → 中间这段 5 → 3 的和就是 8
```

所以在节点 3 处，只需查 map 里有没有前缀和等于 `18 - 8 = 10`：

```
在节点 3 处：prefixSum=18, 查 18-8=10 → map 中有 10（出现 1 次）
→ 说明存在一条路径和为 8（从节点 5 到节点 3）
```

**路径边界（开闭）**：设 X = currSum - targetSum，找到前缀和为 X 的祖先节点后：
...
- 祖先节点本身**不算**在内（它的前缀和被减掉了）
- 当前节点**算**在内
- 特殊情况 X = 0：说明路径从根节点开始 → 这就是初始化 `prefixCount.put(0L, 1)` 的原因

**为什么要回溯**：map 里只应该存"从根到当前节点这一条路径"上的前缀和，不能混入别的分支的。
...
不回溯的后果：
  走完左分支 10→5→3 后，map 里残留了 5 的前缀和 15
  再走右分支到 11 时：prefixSum = 10-3+11 = 18，查 18-8=10 → 命中根（✓ 合法，根是祖先）
  但若某个分支的前缀和恰好等于 currSum-targetSum，就会被误当成"祖先"
  → 实际它根本不在当前路径上，路径必须向下，这样算出来的路径是假的

回溯的做法：
  递归返回前把当前节点的 prefixSum 计数 -1
  → map 里永远只留着"当前这条从根到叶的路径"的前缀和
  → 换分支时旧分支的痕迹已被擦干净
```

对应数组版"和为 K 的子数组"不需要回溯（数组只有一条路线），而树有分支，这是两者的本质区别。
